...
const peoplePanelEl = $('#peoplePanel');
...
function updateTitleBadge() {
  document.title = unreadTotal > 0 ? `(${unreadTotal > 99 ? '99+' : unreadTotal}) Wirely` : 'Wirely';
}
...
function notifyIncoming(senderName, content) {
  ...
  if (window.Notification && Notification.permission === 'granted') {
    try { new Notification(senderName, { body: content, tag: 'wirely-message' }); } catch (_) {}
  }
}
...
// ---------- Ripple effect (lightweight, delegated) ----------

document.addEventListener('click', (e) => {
  const el = e.target.closest('.btn-primary, .fab, .send-btn, .icon-btn, .person-action, .search-result-action, .theme-swatch');
  if (!el) return;
...
function buildThemeGrid() {
  ...
  grid.querySelectorAll('.theme-swatch').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      localStorage.setItem('wirely-theme', id);
...
function connectSocket() {
  ...
  socket.on('contact_request_received', async () => {
    await loadRequests();
  });

  socket.on('contact_request_accepted', async () => {
  ...
  socket.on('typing', ({ fromUserId }) => {
    if (activeContact && activeContact.id === fromUserId) {
      ...
      if (indicator) indicator.textContent = `${activeContact.display_name} is typing…`;
    }
  });
...
function setupTabs() {
  $$('.tab-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      peoplePanelEl.hidden = tab !== 'people';
    });
  });
  ...
  incomingRequests.forEach((r) => {
    ...
    row.innerHTML = `
      ...
      <div class="person-meta">
        <div class="person-name">${escapeHtml(r.display_name)}</div>
...
function setupDiscover() {
  ...
  $('#discoverSortSelect').addEventListener('change', (e) => {
...
async function loadDiscover() {
  ...
  try {
    ...
    discoverListEl.innerHTML = `<div class="no-results">${escapeHtml(err.message)}</div>`;
  }
}

function renderDiscover() {
  if (!discoverCache.length) {
    discoverListEl.innerHTML = `<div class="no-results">No one to show yet.</div>`;
    return;
  }
    ...
    row.innerHTML = `
      ...
      <div class="person-meta">
        <div class="person-name-row">
          <div class="person-name">${escapeHtml(u.display_name)}</div>
        </div>
  ...
  conversationEl.innerHTML = `
    ...
    <form class="composer" id="composerForm">
      ...
      <div class="attach-menu" id="attachMenu">
        <button type="button" data-accept="image/*,video/*">&#128247; Photo &amp; video</button>
        <button type="button" data-accept="">&#128196; Document</button>
      </div>
...
function buildEmojiPicker() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (attachMenu && !attachMenu.contains(e.target) && e.target.id !== 'attachBtn') attachMenu.classList.remove('visible');
  });
}
