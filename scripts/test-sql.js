...
queries.addComment.run(postInfo.lastInsertRowid, bob.id, 'nice post', null);
...
// ---- Beta 2.8: comment replies + reactions, profile extras, search fix ----
const topComment = queries.getComments.all(postInfo.lastInsertRowid)[0]; // the 'nice post' comment added earlier
queries.addComment.run(postInfo.lastInsertRowid, alice.id, 'thanks!', topComment.id);
console.log('reply count on top comment:', queries.countReplies.get(topComment.id).count);

queries.setCommentReaction.run(topComment.id, bob.id, '❤️');
queries.setCommentReaction.run(topComment.id, bob.id, '👍'); // change reaction
console.log('comment reactions:', queries.getReactionsForPost.all(postInfo.lastInsertRowid).map(r => r.emoji));
queries.removeCommentReaction.run(topComment.id, bob.id);
console.log('comment reactions after remove:', queries.getReactionsForPost.all(postInfo.lastInsertRowid).length);

queries.updateProfileExtra.run({ id: alice.id, university: 'University of Pretoria', location_text: 'Pretoria, Gauteng' });
queries.markTourCompleted.run(alice.id);
const aliceExtra = queries.getUserById.get(alice.id);
console.log('profile extras OK:', aliceExtra.university, aliceExtra.location_text, aliceExtra.tour_completed);

const searchByDisplayName = queries.searchUsersByUsername.all({ term: '%Bob%', myId: alice.id });
console.log('search by display_name works:', searchByDisplayName.length === 1);
