...
let postImageUpload = null; // { url, name, mime, size } after upload for the feed composer
let feedCursor = null;
...
const discoverPanelEl = $('#discoverPanel');
...
function closeAllPopovers(exceptEl) {
  $$('.more-menu.visible, .emoji-picker.visible, .attach-menu.visible').forEach((el) => {
    if (el !== exceptEl) el.classList.remove('visible');
  });
}
document.addEventListener('click', (e) => {
  if (!e.target.closest('.more-menu, [data-opens-menu], .emoji-picker, #emojiBtn, .attach-menu, #attachBtn')) {
    closeAllPopovers();
  }
});
...
document.addEventListener('click', (e) => {
  const el = e.target.closest('.btn-primary, .fab, .send-btn, .icon-btn, .person-action, .search-result-action, .theme-swatch, .rail-btn, .composer-box-post');
  if (!el) return;
  ...
  return Array.from({ length: count }).map(() => `
    <div class="post-card">
      <div class="skeleton-row" style="padding:0;">
        <div class="skeleton-circle"></div>
        <div class="skeleton-meta"><div class="skeleton-line w35"></div><div class="skeleton-line w60" style="height:8px;"></div></div>
      </div>
      <div class="skeleton-line w60" style="height:14px;margin-top:14px;"></div>
    </div>`).join('');
}
...
const RAIL_ICONS = { home: 'home', chats: 'chat', people: 'users', notifications: 'bell' };

function setupRail() {
  $$('.rail-btn[data-view]').forEach((btn) => {
    btn.innerHTML = icon(RAIL_ICONS[btn.dataset.view]) + (btn.querySelector('.rail-badge') ? btn.querySelector('.rail-badge').outerHTML : '');
  });
...
function switchView(view) {
  ...
  if (view === 'people' && !discoverCache.length && !discoverPanelEl.hidden) loadDiscover();
  if (view === 'notifications') loadNotifications();
}

function updateRailBadges() {
  const chatsBadge = $('#railChatsBadge');
  chatsBadge.hidden = unreadTotal === 0;
  const peopleBadge = $('#railPeopleBadge');
  peopleBadge.hidden = incomingRequests.length === 0;
  const notifBadge = $('#railNotifBadge');
  notifBadge.hidden = notifUnread === 0;
  updateTitleBadge();
}
...
async function bootstrap() {
  ...
  setupRail();
  renderMyAvatar();
  contactListEl.innerHTML = skeletonRows(5);
  $('#feedList').innerHTML = skeletonPosts(3);
  connectSocket();
  setupTabs();
  setupNewChatModal();
  setupProfileModal();
...
function renderMyAvatar() {
  ...
  $('#myAvatar').style = a.style;
  $('#myAvatar').innerHTML = a.html;
  $('#feedComposerAvatar').style = a.style;
  $('#feedComposerAvatar').innerHTML = a.html;
}
...
function connectSocket() {
  ...
  socket.on('online_snapshot', ({ userIds }) => {
    ...
    if (!discoverPanelEl.hidden) renderDiscover();
  });
  ...
  socket.on('notification', async () => {
    notifUnread += 1;
    updateRailBadges();
    if ($('.view-panel[data-view="notifications"]').classList.contains('active')) loadNotifications();
  });
  ...
  socket.on('new_message', (message) => {
    ...
    else loadContacts(); // chat was hidden/cleared — reload so it reappears
  });
    ...
    if (activeContact && activeContact.id === userId) {
      ...
      if (statusEl) {
        ...
        statusEl.innerHTML = online
          ...
          : `<span class="dot"></span> ${lastSeen ? 'Last seen ' + formatPreviewTime(lastSeen) : '@' + escapeHtml(activeContact.username)}`;
      }
    }
  });
  ...
  socket.on('typing', ({ fromUserId }) => {
    if (activeContact && activeContact.id === fromUserId) {
      ...
      if (indicator) indicator.innerHTML = `<span class="typing-signature"><span class="typing-word">Typing<span class="bolt">${ICONS_BOLT}</span>...</span></span>`;
    }
  });
...
const ICONS_BOLT = '<svg viewBox="0 0 100 100" fill="currentColor"><path d="M58 4 L18 54 L46 54 L40 96 L84 40 L54 40 Z"/></svg>';
...
// ---------- Tabs (People sub-tabs) ----------

function setupTabs() {
  $$('.people-subtab').forEach((btn) => {
    btn.addEventListener('click', () => {
      $$('.people-subtab').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      const sub = btn.dataset.subtab;
      requestsListEl.hidden = sub !== 'requests';
      discoverPanelEl.hidden = sub !== 'discover';
      if (sub === 'discover' && !discoverCache.length) loadDiscover();
    });
  });
}
    ...
    item.innerHTML = `
      ...
      <div class="contact-meta">
        <div class="contact-name-row">
          <div class="contact-name">${escapeHtml(c.display_name)}${c.muted ? ' 🔇' : ''}</div>
...
function renderRequests() {
  ...
  incomingRequests.forEach((r) => {
    ...
    row.innerHTML = `
      ${renderAvatar(r)}
      <div class="person-meta"><div class="person-name">${escapeHtml(r.display_name)}</div><div class="request-tag">@${escapeHtml(r.username)} wants to connect</div></div>
...
function setupDiscover() {
  $('#discoverSearchInput').addEventListener('input', debounce(() => loadDiscover(), 300));
  $('#discoverSortSelect').addEventListener('change', (e) => { discoverSort = e.target.value; loadDiscover(); });
}
...
async function loadDiscover() {
  ...
  try {
    const { users } = await api.get(`/api/users/discover?sort=${discoverSort}&q=${encodeURIComponent(q)}`);
    discoverCache = users;
    renderDiscover();
...
function renderDiscover() {
  if (!discoverCache.length) { discoverListEl.innerHTML = `<div class="no-results">No one to show yet.</div>`; return; }
  ...
  discoverCache.forEach((u) => {
    ...
    row.innerHTML = `
      ...
      <div class="person-meta">
        <div class="person-name-row"><div class="person-name">${escapeHtml(u.display_name)}</div></div>
...
function renderUserSearchResults(users, container, modal) {
  ...
  users.forEach((u) => {
    ...
    row.innerHTML = `${renderAvatar(u)}<div class="search-result-meta"><div class="search-result-name">${escapeHtml(u.display_name)}</div><div class="search-result-username">@${escapeHtml(u.username)}</div></div>${actionHtml}`;
    const btn = row.querySelector('button');
    if (btn && u.relationship === 'accepted') {
      btn.addEventListener('click', async () => {
        modal.hidden = true;
...
function setupProfileModal() {
  ...
  $('#avatarFileInput').addEventListener('change', async (e) => {
    ...
    try {
      ...
      $('#profileError').textContent = err.message; $('#profileError').classList.add('visible');
    }
  });
...
// ---------- Profile view modal (View Profile on a contact) ----------
...
async function openProfileView(userId) {
  ...
  try {
    ...
    body.innerHTML = `
      ...
      <div class="profile-view-name">${escapeHtml(u.display_name)}</div>
      ...
      <div class="profile-view-stats">
        ...
        <div class="profile-view-stat"><b>${toDate(u.created_at).toLocaleDateString([], { month: 'short', year: 'numeric' })}</b><span>Joined</span></div>
        <div class="profile-view-stat"><b>${data.online ? 'Online' : 'Offline'}</b><span>Status</span></div>
      </div>`;
  } catch (err) {
...
async function openSharedMedia(contactId) {
  ...
  modal.hidden = false;
  try {
    const { media } = await api.get(`/api/messages/${contactId}/media`);
    if (!media.length) { grid.innerHTML = `<div class="empty-contacts">No shared media yet.</div>`; return; }
    grid.innerHTML = media.map((m) => m.type === 'image'
...
async function openConversation(contact) {
  ...
  conversationEl.innerHTML = `
    ...
    <div class="messages" id="messages">${skeletonRows(4)}</div>
  ...
  $('#convSearchInput').addEventListener('input', debounce(async (e) => {
    ...
    if (!q) { const { messages } = await api.get(`/api/messages/${contact.id}`); renderMessagesList(messages); return; }
    const { messages } = await api.get(`/api/messages/${contact.id}/search?q=${encodeURIComponent(q)}`);
    renderMessagesList(messages.reverse());
  }, 300));
...
function setupMoreMenu(contact) {
  ...
  $('#menuSearch').addEventListener('click', () => {
    ...
    $('#convSearchBar').classList.add('visible');
  ...
  $('#menuMute').addEventListener('click', async () => {
    ...
    await loadContacts();
    $('#menuMuteLabel').textContent = nowMuted ? 'Unmute' : 'Mute';
  });
  ...
  $('#menuBlock').addEventListener('click', async () => {
    ...
    await loadContacts();
    $('#menuBlockLabel').textContent = nowBlocked ? 'Unblock' : 'Block';
  });
  ...
  $('#menuDeleteChat').addEventListener('click', async () => {
    ...
    activeContact = null;
    conversationEl.innerHTML = `<div class="no-conversation">Select a chat from the list, or find someone new in People.</div>`;
    await loadContacts();
  });
...
function bubbleContentHtml(message) {
  ...
  let mediaHtml = '';
  if (message.type === 'image') mediaHtml = `<img class="attachment-image" src="${escapeHtml(message.attachment_path)}" alt="Shared image" />`;
  else if (message.type === 'voice') mediaHtml = `<div class="attachment-voice"><audio controls src="${escapeHtml(message.attachment_path)}"></audio></div>`;
  else if (message.type === 'file') mediaHtml = `<a class="attachment-file" href="${escapeHtml(message.attachment_path)}" target="_blank" rel="noopener"><span class="attachment-file-icon">${icon('file')}</span><span><span class="attachment-file-name">${escapeHtml(message.attachment_name || 'File')}</span><br><span class="attachment-file-size">${fileSizeLabel(message.attachment_size)}</span></span></a>`;
...
function handleMessageAction(action, message) {
  if (action === 'reply') startReply(message);
    ...
    if (confirm('Delete this message?')) socket.emit('delete_message', { messageId: message.id }, (res) => { if (res && !res.ok) alert(res.error); });
  }
}

function startReply(message) {
  replyingTo = message; editingMessageId = null;
  const bar = $('#replyBar'); bar.classList.add('visible');
...
function syncPostButton() {
  const hasContent = $('#postContentInput').value.trim().length > 0 || !!postImageUpload;
...
async function submitPost() {
  ...
  try {
    ...
    postImageUpload = null; $('#postImagePreview').classList.remove('visible');
    $('#feedList').insertAdjacentHTML('afterbegin', renderPostCard(post));
...
async function loadFeed(reset) {
  ...
  feedLoading = true;
  if (reset) { feedCursor = null; feedDone = false; $('#feedEnd').hidden = true; }

  try {
    const { posts, nextCursor } = await api.get(`/api/posts?limit=10${feedCursor ? '&cursor=' + feedCursor : ''}`);
    if (reset) $('#feedList').innerHTML = '';
    posts.forEach((post) => { $('#feedList').insertAdjacentHTML('beforeend', renderPostCard(post)); wirePostCard(post.id); });
    feedCursor = nextCursor;
    if (!nextCursor) { feedDone = true; $('#feedEnd').hidden = false; }
  } catch (err) {
    if (reset) $('#feedList').innerHTML = `<div class="empty-contacts">${escapeHtml(err.message)}</div>`;
  } finally {
    feedLoading = false;
  }
}
    ...
    <div class="post-card" data-id="${post.id}">
      <div class="post-card-header">
        ...
        <div class="post-card-meta">
          <div class="post-card-name">${escapeHtml(post.author.display_name)}</div>
          <div class="post-card-time">${formatRelativeTime(post.created_at)}${post.edited_at ? ' · edited' : ''}</div>
        </div>
        ${mine ? `<button class="post-card-menu-btn" data-role="menu">${icon('more')}</button>` : ''}
      </div>
      <div class="post-card-content" data-role="content">${escapeHtml(post.content)}</div>
      ${post.image_path ? `<img class="post-card-image" src="${escapeHtml(post.image_path)}" alt="" />` : ''}
      <div class="post-card-footer">
        <button class="post-action ${post.liked_by_me ? 'liked' : ''}" data-role="like">${icon(post.liked_by_me ? 'heartFilled' : 'heart')} <span data-role="like-count">${post.like_count}</span></button>
        <button class="post-action" data-role="comment-toggle">${icon('comment')} <span data-role="comment-count">${post.comment_count}</span></button>
      </div>
      <div class="post-comments" data-role="comments">
        <div data-role="comment-list"></div>
        <div class="comment-input-row"><input type="text" placeholder="Write a comment…" data-role="comment-input" maxlength="500" /><button data-role="comment-submit">Send</button></div>
      </div>
    </div>`;
}
...
function wirePostCard(postId) {
  ...
  likeBtn.addEventListener('click', async () => {
    ...
    } catch (err) { /* revert on failure */ likeBtn.classList.toggle('liked', liked); }
  });
  ...
  commentToggle.addEventListener('click', async () => {
    ...
    if (commentsEl.classList.contains('visible')) await loadComments(postId);
  });
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      input.value = '';
      card.querySelector('[data-role="comment-count"]').textContent = res.comment_count;
      renderComments(postId, res.comments);
    } catch (err) { alert(err.message); }
  };
  card.querySelector('[data-role="comment-submit"]').addEventListener('click', submitComment);
  card.querySelector('[data-role="comment-input"]').addEventListener('keydown', (e) => { if (e.key === 'Enter') submitComment(); });

  const menuBtn = card.querySelector('[data-role="menu"]');
  if (menuBtn) {
    menuBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (confirm('Post options:\nOK = Edit, Cancel = Delete')) {
        const contentEl = card.querySelector('[data-role="content"]');
        const newText = prompt('Edit your post:', contentEl.textContent);
        if (newText !== null && newText.trim()) {
          api.patch(`/api/posts/${postId}`, { content: newText.trim() })
            .then(({ post }) => { contentEl.textContent = post.content; })
            .catch((err) => alert(err.message));
        }
      } else {
        if (confirm('Delete this post?')) {
          api.del(`/api/posts/${postId}`).then(() => card.remove()).catch((err) => alert(err.message));
        }
      }
    });
  }
...
function renderComments(postId, comments) {
  ...
  list.innerHTML = comments.map(c => `
    <div class="comment-row">${renderAvatar(c)}<div class="comment-bubble"><b>${escapeHtml(c.display_name)}</b>${escapeHtml(c.content)}</div></div>
  `).join('');
}
...
const NOTIF_ICON = { friend_request: 'users', friend_accept: 'users', like: 'heartFilled', comment: 'comment', message: 'chat' };
...
async function loadNotifications() {
  try {
    ...
    if (!notifications.length) { list.innerHTML = `<div class="empty-contacts">No notifications yet.</div>`; return; }
    list.innerHTML = notifications.map(n => `
      <div class="notif-card ${n.read ? '' : 'unread'}">
        <div class="notif-icon">${icon(NOTIF_ICON[n.type] || 'bell')}</div>
