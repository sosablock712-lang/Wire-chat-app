...
function serializeComments(postId) {
  const rows = queries.getComments.all(postId);
  const reactionRows = queries.getReactionsForPost.all(postId);
  const byComment = {};
  reactionRows.forEach((r) => { byComment[r.comment_id] = byComment[r.comment_id] || []; byComment[r.comment_id].push({ user_id: r.user_id, emoji: r.emoji }); });
  return rows.map((c) => ({
    id: c.id,
    post_id: c.post_id,
    author: { id: c.author_id, username: c.username, display_name: c.display_name, avatar_path: c.avatar_path, badge: c.badge },
    content: c.content,
    created_at: c.created_at,
    parent_comment_id: c.parent_comment_id,
    parent_author_name: c.parent_author_name || null,
    reply_count: queries.countReplies.get(c.id).count,
    reactions: byComment[c.id] || []
  }));
}
...
router.get('/:id/comments', (req, res) => {
  const postId = parseInt(req.params.id, 10);
  res.json({ comments: serializeComments(postId) });
});

// POST /api/posts/:id/comments { content, parentCommentId }
router.post('/:id/comments', postLimiter, (req, res) => {
  ...
  const parentCommentId = Number.isInteger(req.body?.parentCommentId) ? req.body.parentCommentId : null;
  ...
  if (parentCommentId) {
    const parent = queries.getCommentById.get(parentCommentId);
    if (!parent || parent.post_id !== postId) return res.status(400).json({ error: 'Invalid reply target.' });
    parentAuthorId = parent.author_id;
  }

  queries.addComment.run(postId, req.userId, content, parentCommentId);
  ...
  if (parentAuthorId && parentAuthorId !== req.userId && parentAuthorId !== post.author_id) {
    queries.createNotification.run({ user_id: parentAuthorId, type: 'comment', actor_id: req.userId, post_id: postId, preview: content.slice(0, 80) });
    const io = req.app.get('io');
    if (io) io.to(`user:${parentAuthorId}`).emit('notification', { type: 'comment' });
  }

  res.status(201).json({ comments: serializeComments(postId), comment_count: queries.countComments.get(postId).count });
});

// POST /api/posts/:postId/comments/:commentId/react { emoji }
router.post('/:postId/comments/:commentId/react', (req, res) => {
  const postId = parseInt(req.params.postId, 10);
  const commentId = parseInt(req.params.commentId, 10);
  const emoji = typeof req.body?.emoji === 'string' ? req.body.emoji.trim().slice(0, 8) : '';
  const comment = queries.getCommentById.get(commentId);
  if (!comment || comment.post_id !== postId) return res.status(404).json({ error: 'Comment not found.' });
  if (!emoji) return res.status(400).json({ error: 'Invalid reaction.' });

  queries.setCommentReaction.run(commentId, req.userId, emoji);

  if (comment.author_id !== req.userId) {
    const io = req.app.get('io');
    if (io) io.to(`user:${comment.author_id}`).emit('notification', { type: 'like' });
  }
  res.json({ comments: serializeComments(postId) });
});

// DELETE /api/posts/:postId/comments/:commentId/react
router.delete('/:postId/comments/:commentId/react', (req, res) => {
  const postId = parseInt(req.params.postId, 10);
  const commentId = parseInt(req.params.commentId, 10);
  queries.removeCommentReaction.run(commentId, req.userId);
  res.json({ comments: serializeComments(postId) });
});
