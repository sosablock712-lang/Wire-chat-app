...
function badgeHtml(badge) {
  ...
  // The colored circle + background gradient already come from the
  // existing .badge-chip / .badge-chip.<type> CSS (untouched). This only
  // supplies the glyph that sits centered inside it — a clean white icon
  // instead of a plain text character, distinct per badge type.
  const icons = {
    blue_check: `<svg viewBox="0 0 16 16" width="11" height="11"><path d="M3 8.3l3.3 3.3L13 4.4" stroke="#fff" stroke-width="2.1" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
    red_check: `<svg viewBox="0 0 16 16" width="11" height="11"><path d="M8 1l1.8 3.6 4 .6-2.9 2.8.7 4-3.6-1.9-3.6 1.9.7-4L2.2 5.2l4-.6L8 1Z" fill="#fff"/></svg>`,
    gold_lightning: `<svg viewBox="0 0 16 16" width="11" height="11"><path d="M8.8 1.3 3.6 8.7h2.9L5.7 14.7l6-7.4H8.7l0.1-6z" fill="#fff"/></svg>`
  };

  return `<span class="badge-chip ${badge}">${icons[badge] || icons.blue_check}</span>`;
}
...
// Lightweight, reusable pull-to-refresh: only engages when the container is
// already scrolled to the top, shows a small spinner while refreshing, and
// snaps back regardless of success/failure.
function attachPullToRefresh(containerEl, onRefresh) {
  if (!containerEl || containerEl.dataset.ptrAttached) return;
  containerEl.dataset.ptrAttached = '1';

  const indicator = document.createElement('div');
  indicator.className = 'ptr-indicator';
  indicator.innerHTML = `<span class="ptr-spinner"></span>`;
  // Re-inserted defensively on every touchstart below, because the lists
  // this attaches to (#contactList, #notifList, #feedListV27's scroller,
  // #peopleScroll) get their innerHTML fully replaced on every data reload
  // — a plain one-time prepend() would get wiped out the first time the
  // list refreshes.
  containerEl.prepend(indicator);

  let startY = null, pulling = false, refreshing = false;

  containerEl.addEventListener('touchstart', (e) => {
    if (indicator.parentNode !== containerEl) containerEl.prepend(indicator);
    if (containerEl.scrollTop <= 0 && !refreshing) { startY = e.touches[0].clientY; pulling = true; }
  }, { passive: true });

  containerEl.addEventListener('touchmove', (e) => {
    if (!pulling || refreshing) return;
    const dy = e.touches[0].clientY - startY;
    if (dy > 0 && containerEl.scrollTop <= 0) {
      const pull = Math.min(70, dy * 0.5);
      indicator.style.height = pull + 'px';
      indicator.classList.toggle('ready', pull > 50);
    }
  }, { passive: true });
...
// Reverse geocodes coordinates into a human-readable "City, Region" label
// using OpenStreetMap's free Nominatim API (no API key needed). Falls back
// to a generic label if the lookup fails or times out — raw coordinates are
...
let prevBadgeCounts = { chats: 0, people: 0, notif: 0 };
...
function updateNavBadges() {
  ...
  if (unreadTotal > prevBadgeCounts.chats) pulseBadge(chatsBadge);
  if (incomingRequests.length > prevBadgeCounts.people) pulseBadge(peopleBadge);
  if (notifUnread > prevBadgeCounts.notif) pulseBadge(notifBadge);
  prevBadgeCounts = { chats: unreadTotal, people: incomingRequests.length, notif: notifUnread };
  updateTitleBadge();
}
...
async function bootstrap() {
  ...
  attachPullToRefresh($('#feedScrollV27'), () => loadFeed(true));
  attachPullToRefresh($('#contactList'), () => loadContacts());
  attachPullToRefresh($('#peopleScroll'), () => Promise.all([loadRequests(), loadDiscover()]));
  attachPullToRefresh($('#notifList'), () => loadNotifications());

  if (!me.onboarded) {
    startOnboarding();
  } else {
    if (!me.tour_completed) setTimeout(startTour, 400);
...
function renderContacts(filter = '') {
  ...
  if (!list.length) {
    contactListEl.innerHTML = `<div class="empty-contacts">${filter ? 'No chats match your search.' : 'No conversations yet. Start chatting from the People tab.'}</div>`;
    return;
  }
...
function renderRequests() {
  ...
  incomingRequests.forEach((r) => {
    ...
    row.innerHTML = `${renderAvatar(r)}<div class="person-meta" data-role="open-profile" style="cursor:pointer;"><div class="person-name">${escapeHtml(r.display_name)}${badgeHtml(r.badge)}</div><div class="request-tag">@${escapeHtml(r.username)} wants to connect</div></div>
    ...
    row.querySelector('[data-role="open-profile"]').addEventListener('click', () => openProfileView(r.id));
  ...
  outgoingRequests.forEach((r) => {
    ...
    row.addEventListener('click', () => openProfileView(r.id));
    requestsListEl.appendChild(row);
  });
}
...
function renderDiscover() {
  ...
  if (!discoverCache.length) {
    discoverListEl.innerHTML = `<div class="empty-state">${icon('users')}<h3>Discover people</h3><p>Discover people and grow your network.</p></div>`;
    return;
  }
...
function renderUserSearchResults(users, container, modal) {
  ...
  users.forEach((u) => {
    ...
    row.innerHTML = `${renderAvatar(u)}<div class="search-result-meta" data-role="open-profile" style="cursor:pointer;"><div class="search-result-name">${escapeHtml(u.display_name)}${badgeHtml(u.badge)}</div><div class="search-result-username">@${escapeHtml(u.username)}</div></div>${actionHtml}`;
    row.querySelector('[data-role="open-profile"]').addEventListener('click', () => openProfileView(u.id));
    const btn = row.querySelector('button');
    if (btn && u.relationship === 'accepted') {
      btn.addEventListener('click', async () => {
...
// ---------- Generic people list modal (followers/following) ----------

function setupPeopleListModal() {
  const modal = $('#peopleListModal');
  $('#closePeopleListModal').innerHTML = icon('x');
  $('#closePeopleListModal').addEventListener('click', () => { modal.hidden = true; });
  modal.addEventListener('click', (e) => { if (e.target === modal) modal.hidden = true; });
}
async function openPeopleListModal(userId, kind) {
  const modal = $('#peopleListModal');
  const body = $('#peopleListBody');
  $('#peopleListTitle').textContent = kind === 'followers' ? 'Followers' : 'Following';
  body.innerHTML = skeletonRows(4);
  modal.hidden = false;
  try {
    const { users } = await api.get(`/api/users/${userId}/${kind}`);
    if (!users.length) { body.innerHTML = `<div class="empty-state"><p>No one to show yet.</p></div>`; return; }
    ...
    body.querySelectorAll('.search-result-item').forEach((el) => el.addEventListener('click', () => { modal.hidden = true; openProfileView(parseInt(el.dataset.id, 10)); }));
  } catch (err) { body.innerHTML = `<div class="empty-state"><p>${escapeHtml(err.message)}</p></div>`; }
}
...
async function openProfileView(userId) {
  ...
  body.innerHTML = `<div style="padding:10px;">${skeletonRows(1)}<div class="skeleton-line w60" style="height:14px;margin:16px auto;width:70%;"></div><div class="skeleton-line w35" style="height:34px;margin:0 auto;width:90%;"></div></div>`;
  modal.hidden = false;
  try {
    const data = await api.get(`/api/users/${userId}/profile`);
    const u = data.user;
    let interests = [];
    try { interests = JSON.parse(u.interests || '[]'); } catch (_) {}
    const statusLabel = data.online
      ? `<span class="status-pill online"><span class="dot"></span>Online</span>`
      : `<span class="status-pill"><span class="dot"></span>${u.last_seen ? 'Last seen ' + formatPreviewTime(u.last_seen) : 'Offline'}</span>`;
    const joinLabel = toDate(u.created_at).toLocaleDateString([], { month: 'long', year: 'numeric' });

    body.innerHTML = `
      <div class="pv2-header">
        <div class="pv2-cover"></div>
        ${renderAvatar(u, 'avatar-lg pv2-avatar')}
      </div>
      ...
      <div style="text-align:center;margin-top:6px;">${statusLabel}</div>
      ${u.bio ? `<div class="profile-view-bio">${escapeHtml(u.bio)}</div>` : ''}
      <div class="pv2-meta-row">
        ${u.university ? `<span class="pv2-meta-chip">${icon('interests')} ${escapeHtml(u.university)}</span>` : ''}
        ${u.location_text ? `<span class="pv2-meta-chip">${icon('location')} ${escapeHtml(u.location_text)}</span>` : ''}
        <span class="pv2-meta-chip">${icon('cake')} Joined ${joinLabel}</span>
      </div>
      ${interests.length ? `<div class="pv2-interests">${interests.map(i => `<span class="onboarding-interest-chip selected" style="padding:5px 12px;font-size:11.5px;">${escapeHtml(i)}</span>`).join('')}</div>` : ''}
      <div class="profile-view-stats">
        <div class="profile-view-stat" data-role="stat-friends"><b>${data.friend_count}</b><span>Friends</span></div>
        <div class="profile-view-stat" data-role="stat-followers"><b>${data.follower_count}</b><span>Followers</span></div>
        <div class="profile-view-stat" data-role="stat-following"><b>${data.following_count}</b><span>Following</span></div>
        <div class="profile-view-stat" data-role="stat-posts"><b>${data.post_count}</b><span>Posts</span></div>
      </div>
      ...
      <div class="pv2-primary-actions">
        <button class="btn-primary" style="margin:0;" id="pvFollowBtn">${data.is_following ? 'Following' : 'Follow'}</button>
        ${data.relationship === 'accepted' ? `<button class="btn-secondary" id="pvMessageBtn">Message</button>` : ''}
      </div>
      <button class="btn-secondary" id="pvViewFullBtn" style="width:100%;margin-top:8px;">View Full Profile</button>
      <div class="pv2-icon-actions">
        <button id="pvShareBtn" title="Share">${icon('share')}<span>Share</span></button>
        <button id="pvReportBtn" title="Report">${icon('flag')}<span>Report</span></button>
        <button id="pvBlockBtn" title="${data.is_blocked_by_me ? 'Unblock' : 'Block'}">${icon('block')}<span>${data.is_blocked_by_me ? 'Unblock' : 'Block'}</span></button>
      </div>
      <div class="profile-posts-grid" id="pvPostsGrid" hidden></div>
    `;
    $('#pvFollowBtn').addEventListener('click', async () => {
      try { await api[data.is_following ? 'del' : 'post'](`/api/users/${userId}/follow`); openProfileView(userId); } catch (err) { alert(err.message); }
    });
    ...
    $('#pvShareBtn').addEventListener('click', () => {
      const url = `wirely.app/@${u.username}`;
      if (navigator.share) navigator.share({ title: `${u.display_name} on Wirely`, text: url }).catch(() => {});
      else { navigator.clipboard && navigator.clipboard.writeText(url); alert('Profile link copied.'); }
    });
    $('#pvReportBtn').addEventListener('click', () => openReportModal('user', userId));
    $('#pvBlockBtn').addEventListener('click', async () => {
      const nowBlocked = !data.is_blocked_by_me;
      if (nowBlocked && !confirm(`Block ${u.display_name}? They won't be able to message you.`)) return;
      try { await api[nowBlocked ? 'post' : 'del'](`/api/users/${userId}/block`); await loadContacts(); modal.hidden = true; } catch (err) { alert(err.message); }
    });
    $('#pvViewFullBtn').addEventListener('click', async () => {
      const grid = $('#pvPostsGrid');
      const btn = $('#pvViewFullBtn');
      if (!grid.hidden) { grid.hidden = true; btn.textContent = 'View Full Profile'; return; }
      grid.hidden = false;
      ...
      try {
        ...
        grid.innerHTML = userPosts.length ? userPosts.map(p => `
          <div class="grid-post" data-id="${p.id}">${p.image_path ? `<img src="${escapeHtml(p.image_path)}" alt="" />` : `<div class="text-post-preview">${escapeHtml(p.content).slice(0, 120)}</div>`}</div>`).join('')
          : `<div class="empty-state" style="grid-column:1/-1;padding:20px;"><p>No posts yet.</p></div>`;
        grid.querySelectorAll('.grid-post').forEach((el) => el.addEventListener('click', () => {
          modal.hidden = true; switchView('home');
    ...
    $$('.profile-view-stat[data-role]', body).forEach((el) => {
      el.style.cursor = 'pointer';
      el.addEventListener('click', () => {
        if (el.dataset.role === 'stat-posts') $('#pvViewFullBtn').click();
        else if (el.dataset.role === 'stat-followers') openPeopleListModal(userId, 'followers');
        else if (el.dataset.role === 'stat-following') openPeopleListModal(userId, 'following');
        else if (el.dataset.role === 'stat-friends') { modal.hidden = true; switchView('people'); }
      });
    });
  } catch (err) {
...
function setupLightbox() {
  ...
  function resetZoom() { lbZoom = { scale: 1, x: 0, y: 0 }; snapTransform(); }
  window.__lightboxResetZoom = resetZoom;

  function dist(t1, t2) { return Math.hypot(t2.clientX - t1.clientX, t2.clientY - t1.clientY); }

  img.addEventListener('touchstart', (e) => {
    if (e.touches.length === 2) {
      lbGesture = { type: 'pinch', startDist: dist(e.touches[0], e.touches[1]), startScale: lbZoom.scale };
    } else if (e.touches.length === 1) {
      const now = Date.now();
      if (now - lastTapTime < 300) {
        // Double-tap: toggle zoom
        lbZoom = lbZoom.scale > 1 ? { scale: 1, x: 0, y: 0 } : { scale: 2.4, x: 0, y: 0 };
        snapTransform();
        lbGesture = null;
        lastTapTime = 0;
        return;
      ...
      lastTapTime = now;
      lbGesture = {
        type: lbZoom.scale > 1 ? 'pan' : 'swipe',
        startX: e.touches[0].clientX, startY: e.touches[0].clientY,
        originX: lbZoom.x, originY: lbZoom.y
      };
    }
  }, { passive: true });
  ...
  img.addEventListener('touchmove', (e) => {
    if (!lbGesture) return;
    if (lbGesture.type === 'pinch' && e.touches.length === 2) {
      const newDist = dist(e.touches[0], e.touches[1]);
      lbZoom.scale = Math.min(4, Math.max(1, lbGesture.startScale * (newDist / lbGesture.startDist)));
      applyTransform();
    } else if (lbGesture.type === 'pan' && e.touches.length === 1) {
      lbZoom.x = lbGesture.originX + (e.touches[0].clientX - lbGesture.startX);
      lbZoom.y = lbGesture.originY + (e.touches[0].clientY - lbGesture.startY);
      applyTransform();
    } else if (lbGesture.type === 'swipe' && e.touches.length === 1) {
      const dy = e.touches[0].clientY - lbGesture.startY;
      if (Math.abs(dy) > 10) {
        img.style.transition = 'none';
        img.style.transform = `translateY(${dy}px) scale(${Math.max(0.7, 1 - Math.abs(dy) / 600)})`;
        $('#lightbox').style.background = `rgba(0,0,0,${Math.max(0.3, 0.92 - Math.abs(dy) / 400)})`;
      }
    }
  }, { passive: true });
  ...
  img.addEventListener('touchend', (e) => {
    if (!lbGesture) return;
    if (lbGesture.type === 'swipe') {
      const dy = (e.changedTouches[0]?.clientY ?? lbGesture.startY) - lbGesture.startY;
      const dx = (e.changedTouches[0]?.clientX ?? lbGesture.startX) - lbGesture.startX;
      if (Math.abs(dy) > 110) { closeLightbox(); }
    ...
    } else if (lbGesture.type === 'pan') {
      // clamp pan within reasonable bounds so the image doesn't drift off-screen
      const maxPan = 150 * lbZoom.scale;
      lbZoom.x = Math.min(maxPan, Math.max(-maxPan, lbZoom.x));
      lbZoom.y = Math.min(maxPan, Math.max(-maxPan, lbZoom.y));
      snapTransform();
    }
    lbGesture = null;
  });
...
function moveLightbox(delta) {
  ...
  if (window.__lightboxResetZoom) window.__lightboxResetZoom();
  renderLightboxImage();
}
...
function setupFeed() {
  ...
  const FEELINGS = [
    ...
    { emoji: '😴', label: 'tired' }, { emoji: '🎉', label: 'celebrating' }, { emoji: '🤔', label: 'thoughtful' },
    { emoji: '😤', label: 'motivated' }, { emoji: '😎', label: 'cool' }, { emoji: '😡', label: 'angry' }
  ];
  const feelingPicker = $('#feelingPicker');
  feelingPicker.innerHTML = FEELINGS.map(f => `<button type="button" data-emoji="${f.emoji}" data-label="${f.label}" style="justify-content:flex-start;">${f.emoji} ${f.label}</button>`).join('');
  $('#postFeelingBtn').addEventListener('click', () => { closeAllPopovers(feelingPicker); feelingPicker.classList.toggle('visible'); });
  feelingPicker.querySelectorAll('button[data-emoji]').forEach((btn) => {
    btn.addEventListener('click', () => {
      feelingPicker.querySelectorAll('button').forEach(b => b.style.background = 'none');
      btn.style.background = 'color-mix(in srgb, var(--accent) 15%, transparent)';
      postFeeling = `${btn.dataset.emoji} ${btn.dataset.label}`;
      $('#postFeelingTag').innerHTML = `${icon('smile')} Feeling ${escapeHtml(btn.dataset.label)}`;
      $('#postFeelingTag').classList.add('visible');
  ...
  $('#postLocationBtn').addEventListener('click', () => {
    ...
    $('#postLocationTag').innerHTML = `${icon('location')} Finding your location…`;
    $('#postLocationTag').classList.add('visible');
    navigator.geolocation.getCurrentPosition(async (pos) => {
      const label = await reverseGeocode(pos.coords.latitude, pos.coords.longitude);
      postLocation = label;
      $('#postLocationTag').innerHTML = `${icon('location')} ${escapeHtml(label)}`;
      syncPostButton();
    }, () => {
...
async function loadFeed(reset) {
  ...
  try {
    ...
    if (reset && !posts.length && !nextCursor) {
      $('#feedListV27').innerHTML = `<div class="empty-state">${icon('home')}<h3>Welcome to Wirely 👋</h3><p>Follow people to personalize your feed.</p></div>`;
    } else {
...
function closeCommentsSheet() { $('#commentsSheetOverlay').classList.remove('visible'); activePostForSheet = null; replyingToComment = null; }

function commentReactionSummary(reactions) {
  if (!reactions || !reactions.length) return { chips: [], mine: null };
  const grouped = {};
  reactions.forEach(r => { grouped[r.emoji] = grouped[r.emoji] || []; grouped[r.emoji].push(r.user_id); });
  const mine = reactions.find(r => r.user_id === me.id);
  return { chips: Object.entries(grouped).map(([emoji, users]) => ({ emoji, count: users.length })), mine: mine ? mine.emoji : null };
}

function renderSheetComments(comments) {
  const body = $('#commentsSheetBody');
  if (!comments.length) { body.innerHTML = `<div class="empty-state" style="padding:30px 10px;">${icon('comment')}<p>No comments yet. Be the first to say something.</p></div>`; return; }

  const topLevel = comments.filter(c => !c.parent_comment_id);
  const repliesFor = (id) => comments.filter(c => c.parent_comment_id === id);

  function renderComment(c, isReply) {
    const { chips, mine } = commentReactionSummary(c.reactions);
    const replyLabel = isReply && c.parent_author_name ? `<span class="reply-to-label">replied to ${escapeHtml(c.parent_author_name)}</span>` : '';
    return `
      <div class="sheet-comment-row ${isReply ? 'is-reply' : ''}" data-comment-id="${c.id}">
        <div data-role="open-profile" data-author="${c.author.id}" style="cursor:pointer;">${renderAvatar(c.author)}</div>
        <div class="sheet-comment-bubble">
          <b data-role="open-profile" data-author="${c.author.id}" style="cursor:pointer;">${escapeHtml(c.author.display_name)}${badgeHtml(c.author.badge)} ${replyLabel}</b>
          <div class="sheet-comment-text">${escapeHtml(c.content)}</div>
          ${chips.length ? `<div class="comment-reaction-chips">${chips.map(ch => `<span class="reaction-chip ${ch.emoji === mine ? 'mine' : ''}" data-emoji="${ch.emoji}" data-comment="${c.id}">${ch.emoji} ${ch.count > 1 ? ch.count : ''}</span>`).join('')}</div>` : ''}
          <div class="sheet-comment-actions">
            <button data-role="like-comment" data-comment="${c.id}" class="${mine === '❤️' ? 'active' : ''}">❤️ Like</button>
            <button data-role="react-comment" data-comment="${c.id}">React</button>
            <button data-role="reply-comment" data-comment="${c.id}" data-name="${escapeHtml(c.author.display_name)}">Reply</button>
  ...
  topLevel.forEach((c) => {
    ...
    if (replies.length) html += `<div class="comment-replies">${replies.map(r => renderComment(r, true)).join('')}</div>`;
  });
  body.innerHTML = html;

  body.querySelectorAll('[data-role="open-profile"][data-author]').forEach((el) => {
    el.addEventListener('click', () => openProfileView(parseInt(el.dataset.author, 10)));
  });

  body.querySelectorAll('[data-role="like-comment"]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const commentId = parseInt(btn.dataset.comment, 10);
      const isActive = btn.classList.contains('active');
      try {
        const res = await api[isActive ? 'del' : 'post'](`/api/posts/${activePostForSheet}/comments/${commentId}/react`, isActive ? undefined : { emoji: '❤️' });
        renderSheetComments(res.comments);
        vibrate(15);
      } catch (err) { /* ignore */ }
    });
  });
  body.querySelectorAll('[data-role="react-comment"]').forEach((btn) => {
    btn.addEventListener('click', (e) => showCommentReactionPicker(parseInt(btn.dataset.comment, 10), e.currentTarget));
  });
  body.querySelectorAll('.reaction-chip[data-comment]').forEach((chip) => {
    chip.addEventListener('click', async () => {
      const commentId = parseInt(chip.dataset.comment, 10);
      const mine = chip.classList.contains('mine');
      try {
        const res = await api[mine ? 'del' : 'post'](`/api/posts/${activePostForSheet}/comments/${commentId}/react`, mine ? undefined : { emoji: chip.dataset.emoji });
        renderSheetComments(res.comments);
      } catch (err) { /* ignore */ }
    });
  });
  body.querySelectorAll('[data-role="reply-comment"]').forEach((btn) => {
    btn.addEventListener('click', () => {
      replyingToComment = { id: parseInt(btn.dataset.comment, 10), name: btn.dataset.name };
      $('#commentSheetInput').placeholder = `Reply to ${btn.dataset.name}…`;
      $('#commentSheetInput').focus();
    });
  });
}

function showCommentReactionPicker(commentId, anchorEl) {
  closeAllPopovers();
  let picker = $('#commentReactionPicker');
  if (!picker) {
    picker = document.createElement('div');
    picker.className = 'reaction-picker';
    picker.id = 'commentReactionPicker';
  ...
  picker.querySelectorAll('button').forEach((b) => {
    b.addEventListener('click', async () => {
      try {
        const res = await api.post(`/api/posts/${activePostForSheet}/comments/${commentId}/react`, { emoji: b.dataset.emoji });
        renderSheetComments(res.comments);
      } catch (err) { /* ignore */ }
...
async function submitSheetComment() {
  ...
  try {
    const body = { content };
    if (replyingToComment) body.parentCommentId = replyingToComment.id;
    const res = await api.post(`/api/posts/${activePostForSheet}/comments`, body);
    input.value = '';
...
function renderNotifications() {
  ...
  if (!filtered.length) { list.innerHTML = `<div class="empty-state">${icon('bell')}<h3>You're all caught up.</h3><p>Likes, comments, friend activity, and messages will show up here.</p></div>`; return; }
...
function setupMeView() {
  ...
  $$('.profile-stats-row button[data-stat]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      else if (stat === 'followers') openPeopleListModal(me.id, 'followers');
      else if (stat === 'following') openPeopleListModal(me.id, 'following');
    });
  });
  renderMeTabsLabel();
}
...
async function onboardingNext() {
  ...
  try {
    ...
    switchView('people');
    loadDiscover();
    if (!me.tour_completed) setTimeout(startTour, 500);
    if (window.Notification && Notification.permission === 'default') Notification.requestPermission();
  } catch (err) { alert(err.message); }
}

// ---------- First-time app tour (spotlight walkthrough) ----------
...
function buildTourSteps() {
  tourSteps = [
    { type: 'welcome' },
    { target: '.bn-btn[data-view="home"]', title: '🏠 Home', text: 'Your personalized feed of posts from people you follow and your friends.' },
    { target: '.bn-btn[data-view="chats"]', title: '💬 Chats', text: 'All your real-time conversations live here, with read receipts and reactions.' },
    { target: '.bn-btn[data-view="people"]', title: '👥 People', text: 'Find and connect with new people, and manage friend requests.' },
    { target: '.bn-btn[data-view="notifications"]', title: '🔔 Alerts', text: 'Likes, comments, follows, and messages — all in one place.' },
    { target: '.bn-btn[data-view="me"]', title: '👤 Me', text: 'Your profile, posts, saved items, and settings.' },
    { target: '#openSearchBtn', title: 'Search', text: 'Search for people, posts, #hashtags, and @mentions — anytime.', view: 'home' },
    { target: '.composer-card', title: 'Share something', text: 'Tap here to post text, photos, your mood, or your location.', view: 'home' },
    { type: 'finish' }
  ];
}
...
function renderTourStep() {
  ...
  card.hidden = true; welcome.hidden = true; spotlight.classList.remove('center');
  ...
  if (step.type === 'welcome') {
    ...
    welcome.innerHTML = `
      <h2>👋 Welcome to Wirely</h2>
      <p>Swipe up to explore more.</p>
      <div class="tour-arrow">${icon('jumpDown')}</div>
      <button class="btn-primary" id="tourWelcomeNext">Continue</button>
    `;
    $('#tourWelcomeNext').addEventListener('click', nextTourStep);
    return;
  }
  if (step.type === 'finish') {
    spotlight.classList.add('center');
    welcome.hidden = false;
    welcome.innerHTML = `
      <h2>🎉 You're ready to explore Wirely!</h2>
      <p>Have fun connecting, posting, and chatting.</p>
      <button class="btn-primary" id="tourStartExploring">Start Exploring</button>
    `;
    $('#tourStartExploring').addEventListener('click', finishTour);
    return;
  }
  ...
  setTimeout(() => {
    ...
    const cardTop = rect.bottom + 220 < window.innerHeight ? rect.bottom + 14 : Math.max(14, rect.top - 140);
    card.style.top = cardTop + 'px';
    card.style.left = Math.min(window.innerWidth - 316, Math.max(14, rect.left - 60)) + 'px';
...
function nextTourStep() {
  tourStepIndex++;
  if (tourStepIndex >= tourSteps.length) { finishTour(); return; }
  renderTourStep();
}
async function finishTour() {
  $('#tourOverlay').classList.remove('visible');
  try { const { user } = await api.post('/api/users/me/tour-complete'); me = user; } catch (_) {}
}
